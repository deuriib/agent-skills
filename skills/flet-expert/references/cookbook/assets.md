# Assets
URL: https://flet.dev/docs/cookbook/assets
...
For UI controls such as Image, you usually pass a path relative to assets_dir:

ft.Image(src="images/sample.png")


For non-UI file operations (ex: reading JSON, opening a bundled SQLite database, loading ML models, etc.), it is often more convenient to use an absolute path to your assets directory. For this reason, Flet sets the FLET_ASSETS_DIR environment variable (in production apps built with flet build,excluding web) to an absolute path to your assets directory.
...
This gives you a single way to resolve bundled files across local runs and production builds. See FLET_ASSETS_DIR for details.
